...
import SwiperCardsYear from 'components/SwiperCardsYear';
...
export default function Testimonials() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const querySnapshot = await getDocs(collection(db, 'ACE'));
  ...
  const filteredEventData = eventData.filter((event) => !selectedYear || event.year === selectedYear);

  return (
    <div style={{ marginTop: '-10.5%' }}>
      <SwiperCardsYear selectedYear={selectedYear} setSelectedYear={setSelectedYear} />
      <TestimonialsWrapper>
        <Swiper
        ...
        >
          {filteredEventData.map((event, idx) => (
            <SwiperSlide key={idx}>
              <TestimonialCard>
                <Title>{event.eventName}</Title>
                <ImageContainer>
                  <StyledImage src={event.img || '/default-image.jpg'} alt="Testimonials" layout="fill" />
                  <Overlay className="overlay">
                    <Content>“{event.eventDescription}”</Content>
                  </Overlay>
                </ImageContainer>
                <AuthorContainer>
                  <AuthorContent>
                    <AuthorName>{event.endTime}</AuthorName>
                    <AuthorTitle>{event.venue}</AuthorTitle>
                  </AuthorContent>
                </AuthorContainer>
              </TestimonialCard>
            </SwiperSlide>
